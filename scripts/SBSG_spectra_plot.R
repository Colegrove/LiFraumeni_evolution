##### plot the spectra of SBSG for figure 2H



spectra <- read_delim("/Users/huntc10/Desktop/sbs_spectra.txt")
sbsG_data <- spectra %>%
...
sbsG <- ggplot(sbsG_data, aes(x = Context, y = SBSG, fill = Substitution)) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.02)))
sbsG
ggsave("./results/sbsg_spectra.png", sbsG, height = 1.5, width = 1.9, units = "in", dpi = 300)
