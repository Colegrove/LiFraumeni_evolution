...
lolipopComparison_slide <- function(
  ...
  for (GeneIter in unique(forPlot$Gene)) {
    ...
    test_plot <- ggplot() +
      ...
      scale_y_continuous(
        name = paste0("Mutation\ncount"), 
        limits = c(0, ceiling(max(plot_data$Count)) + 1)) +
      theme_bw() +
      theme(
    ...
    domain_plot_list[[GeneIter]] <- list(
      aligned = aligned,
      plot_title = plot_title,
      test_plot = test_plot,
      test_domains_plot = test_domains_plot,
      test_plot_2 = test_plot_2,
