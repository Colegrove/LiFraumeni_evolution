...
region_map <- tribble(
  ~chr,      ~full_region, ~genic, 
  "chr1",    "region_208", "genic/intergenic",
  "chr2",    "region_2896", "genic",
  "chr4",    "region_4173", "genic/intergenic",
  "chr6",    "region_5020", "genic",
  "chr7",    "region_5144", "genic",
  "chr8",    "region_5520", "genic",
  "chr9",    "region_5739", "genic",
  "chr10",   "region_784", "intergenic",
  "chr11",   "region_1111", "genic",
  "chr12",   "region_1355", "intergenic",
  "chr13",   "region_1501", 'genic',
  "chr14",   "region_1725", 'genic',
  "chr15",   "region_1904", "intergenic",
  "chr16",   "region_2115", 'genic/intergenic',
  "chr17",   "region_2378", 'intergenic',
  "chr18",   "region_2457", 'genic',
  "chr19",   "region_2739", 'genic/intergenic',
  "chr20",   "region_3388", 'genic',
  "chr21",   "region_3515", 'intergenic',
  "chr22",   "region_3703", 'intergenic'
) %>%
  mutate(region = as.integer(sub("region_", "", full_region))) %>%
  dplyr::select(chr, region, genic) %>%
...
depth_by_gene_color <- ggplot(chip_depths_no_tp53, aes(x = gene_name, y = meanDP_subject, color = label)) +
  geom_point(size = 1) +
  #geom_errorbar(aes(ymin = minDP_gene, ymax = maxDP_gene), width = 0) +
  theme_minimal() +
...
MUT_mutations <- maf_masked_coding %>% 
  filter(!(Samp %in% samples_all_exclude)) %>%
  left_join(subj_abbr) %>%
  ...
  mutate(region_split = sub("_.*", "", Region),
         region = ifelse(region_split == "region",
                         sub(".*_", "", Region), NA)
  ) %>%
  filter(region_split == "region") %>%
  mutate(region = as.integer(region)) %>%
  left_join(region_map, by=c("region" = "region")) %>% print(width = Inf)

MUT_by_region <- MUT_mutations %>% 
  group_by(region, Tissue, Subject) %>%
  summarise(total_muts = n()) %>%
  print(width = Inf)


MF_by_mut_region <- chip_mut_depths %>%
  left_join(MUT_by_region, by=c("region" = "region", "Tissue_abbr" = "Tissue", "Subject_abbr" = "Subject")) %>%
  mutate(MF = total_muts/totalDP_subject) %>%
  filter(tissue %in% c("PBMC", "Buffy coat")) %>% print()
...
facet_region <- ggplot(
  MF_by_mut_region,
  aes(x = label_sample, y = MF)
) +
  geom_point(
    size  = 2,
    alpha = 0.8
  ) +
  facet_wrap(~ genic, scales = "free_x") +
  labs(
    x = "Sample / Subject",
    y = "Mutation frequency (MF)"
  ) +
  scale_y_log10() +
  theme_minimal() +
  theme(
...
ggsave("./results/MUT_MF_by_region.png", facet_region, width = 12, height = 12)

###### all genic/intergenic regions aggregated
...
MF_by_mut_region_mean <- MF_by_mut_region %>%
  group_by(label_sample, genic) %>%
  summarise(sum_DP = sum(totalDP_subject), 
            sum_muts = sum(total_muts)) %>% 
  mutate(MF = sum_muts/sum_DP,
         label_sample = factor(label_sample, levels = sample_order)) %>% print()


facet_region <- ggplot(
  MF_by_mut_region_mean,
  aes(x = label_sample, y = MF, color=label_sample)
) +
  geom_point(
    size  = 2,
    alpha = 0.8
...
facet_region <- ggplot(
  MF_by_mut_region_mean,
  aes(x = genic, y = MF, color=label_sample)
) +
  geom_point(
    size  = 2,
    alpha = 0.8
...
MF_by_mut_region_mean_group <- MF_by_mut_region_mean %>%
  mutate(
    group = case_when(
      str_detect(label_sample, "\\(LFS") ~ "LFS",
      str_detect(label_sample, "\\(REL") ~ "non-LFS",
      str_detect(label_sample, "\\(CON") ~ "non-LFS",
      TRUE ~ NA_character_
    ),
    group = factor(group, levels = c("non-LFS", "LFS"))
  ) 

wilcox.test(MF ~ group, data = subset(MF_by_mut_region_mean_group, genic == "genic"))
wilcox.test(MF ~ group, data = subset(MF_by_mut_region_mean_group, genic == "genic/intergenic"))
wilcox.test(MF ~ group, data = subset(MF_by_mut_region_mean_group, genic == "intergenic"))



facet_region <- ggplot(
  MF_by_mut_region,
  aes(x = label_sample, y = MF, color = genic)
) +
  geom_point(
    size = 2,
    alpha = 0.8,
...
#depth_by_region <- depth_by_region + theme(legend.position = "none")
...
depth_supp_table <- chip_depths_subject_panelID %>% 
  ...
  dplyr::select(Subject, Tissue, Panel, Chr, Target, genic, meanDP) %>%
...
columns_order <- c("Subject", "Tissue", "Hugo_Symbol", "NCBI_Build", 
                   ...
                   "t_NC", "VAF", "NF",  "IMPACT", "FILTER", "Transcript_ID", 
                   "Consequence")
samples_all_exclude
