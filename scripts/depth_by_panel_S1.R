...
## Supp table 1 sample list

#mutFreq_combined from mutation_frequencies_blood_CHIP_ms_2BC_v2.R
sample_info <- mutFreq_combined %>% left_join(subj_abbr) %>%
  dplyr::select(Subject_abbr, age, LFS, CTx) %>%
  distinct(Subject_abbr, age, LFS, CTx) %>%
  dplyr::rename(Subject = Subject_abbr, Age = age)

out_path <- "/Users/huntc10/Library/Mobile Documents/com~apple~CloudDocs/GenomePhD/Dissertation/proj/li_fraumeni/manuscript/supplemental_data/"
out_file_subjects <- "supplemental_subject_information.csv"
write_csv(sample_info, paste0(out_path,out_file_subjects))
