...
#### To generate depth panel file first run depth_by_panel_supp.R
...
tissue_information_depths_grouped <- tissue_information_depths %>%
  mutate(
    TissueGroup = if_else(Tissue %in% blood_samps | grepl("PBMC", Tissue, ignore.case = TRUE),
                          "Blood",
                          "Solid tissues"),
    # old style e.g. PBMC (CON07)
    # SampleLabel = if_else(Tissue_abbr %in% cancer_sample_abbr, paste0("<span style='color:red;'>", Tissue_abbr, "</span>", " (<b>", Subject_abbr, "</b>)"), 
    #                       paste0(Tissue_abbr, " (<b>", Subject_abbr, "</b>)")),
...
depth_plot_split
ggsave("results/depth_schematic.png", depth_plot_split, height = 2, width = 5.5)
ggsave("results/depth_schematic.png", depth_plot_split, height = 1.9, width = 4.8)
ggsave("/Users/huntc10/Library/CloudStorage/OneDrive-UW/Li-Fraumeni/Manuscript_figures/Fig_1/depth_schematic.png", depth_plot_split, width = 4.8, height = 1.8, units = "in", dpi = 300)

##########
### dot plot version
##########
tissue_information_depths_grouped %>% filter(TissueGroup == "Blood")
depth_plot_split <- ggplot(tissue_information_depths_grouped,
                           aes(x = SampleLabel, y = DP, fill = GenePanel)) +
  #geom_col() +
  geom_point() +
  ...
  facet_grid(cols = vars(TissueGroup),
             scales = "free_x", space = "free_x", switch = "x") +
  labs(y = "Duplex depth", x = NULL, fill = "Gene panel") +
  #scale_y_continuous(breaks = c(0, 1000, 10000, 20000)) +
  theme_minimal() +
  theme(
    strip.text.x = element_blank(),
    axis.text.x.bottom = element_markdown(angle = 90, vjust = 0.5, hjust = 1, size = 8, margin = margin(t=3)),
    axis.text.y = element_text(size=8),
    axis.title.y = element_text(size=8),
    panel.grid.major.x = element_blank(),
    panel.grid.minor = element_blank(),
...
depth_plot_split
ggsave("results/depth_schematic.png", depth_plot_split, height = 1.9, width = 4.5)
