...
dnds_classic <- ggplot(dnds_all_LFS, aes(x = Category, y = dnds, fill = ifelse(Category == "All", "All", "Other"))) +
  ...
  scale_y_continuous(limits = c(-1.3, 6.5), breaks = c(0,1,2,3,4,5,6), labels = c(0,1,2,3,4,5,6)) +
...
dnds_classic <- dnds_classic +
  scale_y_continuous(limits = c(-2, 6.5), breaks = c(0,1,2,3,4,5,6), labels = c(0,1,2,3,4,5,6))
dnds_classic


ggsave("results/dnds_naive_tissues_grouped_ms.png", dnds_classic, width = 3.75, height = 1.5, units = "in", dpi = 300)
ggsave("/Users/huntc10/Library/CloudStorage/OneDrive-UW/Li-Fraumeni/Manuscript_figures/Fig_4/dnds_tissues_grouped_ms_4.png", dnds_classic, width = 3.75, height = 1.5, units = "in", dpi = 300)
