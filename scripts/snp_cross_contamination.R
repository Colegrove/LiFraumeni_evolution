...
ann_wide <- tibble::tibble(
  ...
  Age        = c(25,30,27,25,37,60,76,35,39,69,61),
...
maf_masked_coding %>% filter(Hugo_Symbol == "RUNX1") %>% filter(!is.na(exon_number))
#removed_df %>% filter(!is.na(exon_number)) %>% print(n=Inf)
# maf_masked_coding <- maf_masked_coding %>%
#   filter(!str_detect(FILTER, "NM8\\.0"))
# maf_masked_coding
