...
observed_all_LFS_summary <- observed_all_LFS %>%
  summarise(
    Hugo_Symbol = "TP53",
    Tissue = "All tissues",
    Group = "LFS",
...
observed_all_LFS <- bind_rows(observed_all_LFS, observed_all_LFS_summary)
observed_all_LFS %>% print(n=Inf)

binom.test(x=190, n=190+19,p=0.779)
...
## remove cases where less than 10 total mutations
# dnds_all_LFS <- dnds_all_LFS %>%
#   group_by(GENE) %>%
#   mutate(min_tot = min(syn_obs + nonsyn_obs)) %>%
#   filter(min_tot > 10) %>%
#   select(-min_tot) %>%
#   ungroup()

## remove any cases where no synonymous/non-synonymous
# dnds_all_LFS <- dnds_all_LFS %>%
#   group_by(GENE) %>%
...
n_reps <- 200
df <- dnds_all_LFS %>%
  dplyr::select(GENE, Tissue, Group, synonymous, nonsynonymous, syn_obs, nonsyn_obs,
...
dnds_all_LFS <- dnds_all_LFS %>% left_join(boot_iqr, by=c("GENE", "Tissue"))
dnds_all_LFS
...
dnds_all_LFS %>% print(n=Inf)
dnds_classic <- ggplot(dnds_all_LFS, aes(x = Tissue, y = dnds, fill = ifelse(Tissue == "All tissues", "All", "Other"))) +
  scale_fill_manual(values = c("All" = "#DDCC77", "Other"="#DDDDDD")) +
  ...
  scale_y_continuous(limits = c(-2.85, 23), breaks = c(0,5,10,15,20), labels = c(0,5,10,15,20)) +
  ...
  theme(
    ...
    axis.text.x = element_markdown(angle = 90, vjust = 0.5, hjust = 1, size=8),
...
ggsave("results/dnds_naive_tissues_ms.png", dnds_classic, width = 4.8, height = 2.25, units = "in", dpi = 300)
