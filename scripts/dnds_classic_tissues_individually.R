...
n_reps <- 200
df <- dnds_all_LFS %>%
  select(GENE, Tissue, Group, synonymous, nonsynonymous, syn_obs, nonsyn_obs,
...
dnds_classic <- ggplot(dnds_all_LFS, aes(x = Tissue, y = dnds, fill = ifelse(Tissue == "All tissues", "All", "Other"))) +
  ...
  scale_y_continuous(limits = c(-1, 23), breaks = c(0,5,10,15,20), labels = c(0,5,10,15,20)) +
...
ggsave("results/dnds_naive_tissues_ms.png", dnds_classic, width = 3.9, height = 2.25, units = "in", dpi = 300)
