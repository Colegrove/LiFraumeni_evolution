...
gff <- import("/Users/huntc10/Downloads/gencode.v38.annotation.gff3")
gff <- as_tibble(gff)
annotations <- gff %>% 
  mutate(transcript_id_base = sub("\\..*", "", transcript_id)) %>%
  filter(transcript_id_base %in% transcripts_list) %>% 
  filter(type %in% c("transcript", "CDS") )%>% 
  #dplyr::select(seqnames, start, end, width, type, gene_name, exon_number)
  dplyr::select(seqnames, strand, start, end, width, type, gene_name, exon_number)

write.csv(annotations, "/Volumes/feder-vol1/project/li_fraumeni/scripts/2025-09-30-dnds_prep/CHIP_cds_annotations.csv")
...
############################################################################
## annotate mutations file within genic/CDS region/and masking
############################################################################

maf_masked_coding <- filt_maf_CHIP %>% 
...
###############
### filter mutations for end repair and 8 read mismatch filters
###############
# maf_masked_coding_artifact <- maf_masked_coding %>%
#   filter(!str_detect(FILTER, "\\b(EndRepairFillInArtifact|NM8\\.0)\\b"))

###############
## filter depth for end repair and 8 read mismatch filters
###############
# final_masked_depth_artifact <- final_masked_depth %>%
#   filter(is.na(filter) | !str_detect(filter, "\\b(EndRepairFillInArtifact|NM8\\.0)\\b"))
