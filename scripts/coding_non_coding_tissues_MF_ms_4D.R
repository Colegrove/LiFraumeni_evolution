...
mutFreq_combined_non_coding <- chip_depths_non_coding %>%
  ...
  mutate(
    ...
    age = age_map[subject]) %>%
  dplyr::rename("Subject" = 'subject', "Hugo_Symbol" = "gene_name")
mutFreq_combined_non_coding %>% print(n=Inf)
## write mutation frequencies by gene/subject
...
MF_base
p_other <- ggplot(
  MF_base %>% filter(tissue != "Skin"),
  aes(x = tissue, y = mutFreq, fill = coding)
) +
...
p_skin <- ggplot(
  ...
  # example: zoom Skin y-axis differently
  # scale_y_log10(limits = c(1e-7, 1e-5))  # adjust as needed
  scale_fill_manual(
    values = c("coding" = "black",
               "non-coding-CHIP" = "#999999"),
    labels = c("coding" = "coding", "non-coding-CHIP" = "non-coding")
  ...
  theme(
    ...
    legend.position = "none"  # legend handled by p_other
  )

p_skin


tissue_coding_non_MF_combined <- p_other + p_skin +
  plot_layout(widths = c(6, 0.3))

tissue_coding_non_MF_combined
ggsave("results/Tissues_coding_non_coding_MF_supp_separateSkin.png", tissue_coding_non_MF_combined, width = 6, height = 4, units = "in", dpi = 300)
...
dotplot_coding_non <- MF_base %>%
  filter(coding %in% c("coding", "non-coding-CHIP")) %>%
  ggplot(aes(x = Category, y = mutFreq, fill = coding)) +
  geom_point(position = position_dodge(width = 0.7), 
             size = 2.5, alpha = 0.8, shape = 21, stroke = 0.3) +
  scale_fill_manual(
    values = c("coding" = "black",
               "non-coding-CHIP" = "#999999"),
    labels = c("coding" = "coding", "non-coding-CHIP" = "non-coding")
...
dotplot_coding_non
ggsave("results/Tissues_coding_non_coding_MF_supp_dot.png", dotplot_coding_non, width = 2, height = 4, units = "in", dpi = 300)
...
ggsave("/Users/huntc10/Library/CloudStorage/OneDrive-UW/Li-Fraumeni/Manuscript_figures/Fig_4/TP53_MF_Tissues_coding_non_coding_ms.png", TP53_MF_tissues, width = 3.75, height = 2, units = "in", dpi = 300)
