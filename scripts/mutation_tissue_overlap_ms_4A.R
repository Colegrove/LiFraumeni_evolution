...
ggsave("/Users/huntc10/Library/CloudStorage/OneDrive-UW/Li-Fraumeni/Manuscript_figures/Fig_4/248_heatmap_ms_4G.png", heatmap_codon, width = 3.75, height = 1, units = "in", dpi = 300)
...
mf_mb_scale_prep %>% arrange(desc(MF)) %>% print(n = Inf)
mf_scale <- ggplot(mf_mb_scale_prep, aes(Tissue, " ", fill = MF)) +
  ...
  theme(axis.text.x = element_blank(),
        ...
        legend.margin = margin(0,0,0,-5))
mf_scale
# mb_scale <- ggplot(mf_mb_scale_prep, aes(Tissue, " ", fill = MB)) +
#   geom_tile(height = 1) +
#   scale_x_discrete(limits = levels(skyscraper_prep$Tissue_ordered),
#                    labels = custom_label) +
#   scale_fill_gradient(high = "black", low = "#DDDDDD") +
#   labs(y = "MB", fill = "MB") +
#   base_theme +
#   theme(axis.text.x = element_blank(),
#         #axis.text.x = element_markdown(size =8, angle = 90),
...
ggsave("/Users/huntc10/Library/CloudStorage/OneDrive-UW/Li-Fraumeni/Manuscript_figures/Fig_4/skyscraper_overlap_tissues_ms_4A.png", stacked_plot, width = 3.75, height = 3.25, units = "in", dpi = 300)
