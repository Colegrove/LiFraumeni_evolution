...
unique_p53_family_pbmc <- family_pbmc %>% 
...
p53_reads_family_pbmc <- family_pbmc %>%
  group_by(Subject) %>%
...
mutations_depth <- right_join(DP_summary_coding, unique_p53_family_pbmc, by = "Samp")
mutations_depth <- right_join(mutations_depth, p53_reads_family_pbmc, by = "Subject")
...
ggsave("results/DP_comparisons_family.png", final_plot, width = 10, height = 5, units = "in")
