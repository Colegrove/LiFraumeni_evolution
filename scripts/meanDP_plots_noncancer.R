...
DP_summary_coding %>% print(n=Inf)
patient_noncancer <-
  filt_maf %>%
  filter(Tissue %in% non_cancer_samples) %>%
  filter(coding == "coding") %>%
  filter(Subject == "Patient")

unique_p53_noncancer <- patient_noncancer %>% 
  group_by(Tissue, Samp) %>% 
  summarize(unique_p53_count = n(), .groups = "drop") %>% 
  # complete(
  #   Tissue = non_cancer_samples,
  #   Samp = unique(patient_noncancer$Samp),
  #   fill = list(unique_p53_count = 0)
  # ) %>%
  print(n=Inf)

p53_reads_noncancer <- patient_noncancer %>%
  group_by(Tissue) %>%
...
mutations_depth <- right_join(DP_summary_coding, unique_p53_noncancer, by = "Samp")
mutations_depth <- right_join(mutations_depth, p53_reads_noncancer, by = "Tissue")
...
ggsave("results/DP_comparisons_patient_normal.png", final_plot, width = 10, height = 5, units = "in")
