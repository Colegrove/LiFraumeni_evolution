...
## Load Functions####
source("scripts/01.1.00_LoadFunctions.R")
source("scripts/01.3.00_FigureParameters.R")
source("scripts/01.2.00_ColorScales.R")

# Load translation tables
## Load auxilary tables ####It 
...
# Read in BED for TP53
targ_bed12 <- loadBed(inputs$bed12)
targ_bed6 <- loadBed(inputs$bed6)
maskBed <- loadBed(inputs$maskBed)

# Read in BED for TP53
targ_bed12_CHIP <- loadBed(inputs$bed12_CHIP)
targ_bed6_CHIP <- loadBed(inputs$bed6_CHIP)

# Read in BED for MUT
targ_bed12_MUT <- loadBed(inputs$bed12_MUT)
targ_bed6_MUT <- loadBed(inputs$bed6_MUT)

# Read in BED for CHIP + MUT
targ_bed12_CHIP_MUT <- loadBed(inputs$bed12_CHIP_MUT)
targ_bed6_CHIP_MUT <- loadBed(inputs$bed6_CHIP_MUT)

## cosmic can be removed but to edit OCHC table and hotspot columns
source("scripts/02.2.1_Load_COSMIC.R")
source("scripts/02.2.2_Process_COSMIC.R")

# Convert genome.mut to depth file
source("scripts/02.1.0_Mut2Depth.R")
  # Read in Depth
source("scripts/02.1.1_LoadDepth.R")
## filter out samples that do not contain coding region mean depth requirements 
source("scripts/02.1.2_FilterDepth.R")

# Read in MAF for TP53
## No seshat combining 22OCT24HLC
source("scripts/02.4.1_LoadMaf.R")
source("scripts/02.4.2_FiltMaf.R")

# Read in MAF for CHIP panel
source("scripts/02.4.1_LoadMaf_CHIP.R")
source("scripts/02.4.2_FiltMaf_CHIP.R")
...
## MB plots and multivariate models not needed anymore?
## MB needed in supp?
#source("scripts/MF_MB_TP53_supplement.R")
...
## other
source("scripts/depth_summaries_in_text.R")
